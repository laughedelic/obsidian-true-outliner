...
restores the cursor the operation itself chose — the merge join point, the indented
node's content start, the split point — rather than a mechanical mapping of the pre-edit
cursor that collapses to the end of the whole rewritten region. Covers the shared
recording mechanism keyed on this plugin's structural `userEvent` values (spanning both
the keyboard-grammar and edit-enforcement rewrite dispatch sites), its pass-through
classification, the invariance of undo granularity, and the documented limit at the
first undo/redo pair. Architecture and rationale: the
`fix-redo-cursor-after-structural-ops` change's design.md; evidence and findings:
`docs/research/04-open-questions.md` Q18–Q21.
