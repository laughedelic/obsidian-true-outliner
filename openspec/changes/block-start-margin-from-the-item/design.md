...
indentation falls short of the top, cleared by an ATX heading. This is the stack `parse` builds
...
keeps the rest of the line. The patterns are applied to that; the block's lines stay the raw
...
### D4. Headings stay at column 0

`ATX_RE` and the setext underline are not given the margin, and neither is a `-` or `=` rule in
the paragraph interruption test, which at the margin would be a setext underline. See the
proposal's Non-Goals.

### D5. The seam rules read the same margin

`kindAsWritten(node, margin)` and `tailAsWritten(node, margin)` demote only past
`margin + OPENING_MARGIN`. `normalizeBoundaries` threads the margin down: a list item's children
...
which is found on the way down to it. A heading keeps margin 0 in `kindAsWritten` (D4).
