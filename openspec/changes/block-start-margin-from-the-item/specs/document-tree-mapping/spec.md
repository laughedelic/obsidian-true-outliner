...
A quote, a callout, a thematic break and an HTML block SHALL open where the line's indentation
...
The same margin SHALL apply wherever a block start is tested from inside another block: a list
item's own continuation lines, a paragraph's continuation lines, and a quote's run, which SHALL
also end at a line indented short of the margin.

An ATX heading and a setext underline SHALL keep measuring from column 0: a heading opens a
section, which a list item cannot hold.
