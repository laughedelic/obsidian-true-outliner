...
- [ ] 1.1 Add failing unit cases in `tests/undo-on-abandon.test.ts` that drive the plan through
...
- [ ] 1.2 Add the negative-control counterparts that must keep passing unchanged: a
...
- [ ] 1.3 Pin the RESIDUE family as negative controls across all six measured shapes — unwrap
...
- [ ] 2.1 Add the removal field to `TxPlan` in `src/plugin/grammar.ts` — a change set in the
...
- [ ] 2.2 Give `planFromOp` an explicit per-call form — reversal, residue deletion, or none —
...
- [ ] 2.5 Compute the removal in `insertionPlan` (Shift+Enter's continuation) as a reversal, by
  the same rule.
- [ ] 2.6 Set the form at every `planKey` branch: `splitNode`, `insertSiblingHeading` and the
  ...
  abandonable place.
- [ ] 2.7 Pass the inner plan's removal through unchanged in both branches of
...
- [ ] 3.1 Define the marker that carries the removal edit on a dispatched transaction, owned by
...
- [ ] 4.1 Turn the group 1 cases green and re-run them with the fix disabled (revert the plan
...
- [ ] 4.2 Re-check transaction classification with a removal that RESTORES text rather than
...
- [ ] 4.3 Assert the history shape end to end: the keypress is one entry, the removal is a
...
- [ ] 5.3 Leave the redo limitation exactly as it stands, and note beside it that this change
