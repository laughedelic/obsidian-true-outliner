...
(`docs/research/selection-follow-ups`, "Gap-line cursor transparency": _the escape hatch stays the mode
toggle, not an in-outline-mode exception_).
...
_(Amendment 2026-07-21, real-vault manual pass: the original single-separator merge
...
internal too, and the scenario below now says so.)_
...
#### Scenario: A marker's surplus whitespace is content, and Backspace removes it

- **WHEN** the marker's whitespace run is wider than the one character it needs — `-  a`,
  `- [ ]  bar`, `##  Two` — and the user presses Backspace at the node's content start
- **THEN** the character before the caret is deleted natively, leaving the marker with its
  one space and the node otherwise unchanged; no merge is recognized and no veto is raised
- **AND** the run's inner columns are not places the caret can rest, so the content start
  is the one column the surplus can be removed from, and a second Backspace there — with the
  run down to one character — is the merge intent the scenario above describes
...
- Backspace with the cursor at a node's first content character — deleting backward
  ...
  Nor SHALL a content start whose marker run is wider than one character: the deletion
  there removes the run's surplus and is ordinary editing (see the chrome-transparency
  requirement above).
