...
- [x] 1.1 Rename `renumberOrderedAfterRemoval` to `renumberOrderedAgainst(before, after)`
...
- [x] 1.2 Convert the nine `renumberOrdered` call sites to pass the before-list. For each,
      ...
      `moveSurgery`.
- [x] 1.3 Delete `renumberOrdered`. `lowestNumber` stays as `renumberOrderedAgainst`'s fallback
      for a run with no member present beforehand (D2), and is no longer reachable as a policy in
      its own right.
- [x] 1.4 Rewrite the helper's doc comment for the single rule. State where a run's identity
      ...
      classification it corrects no longer exists. Keep the fallback's rationale and the
      `- p` / `5. a` / `10. kid` / `6. b` worked example that earned it.
- [x] 1.5 Update the four call-site comments that justify a policy by shape — indent's
...
- [x] 2.1 In `tests/ops.test.ts`, beside the existing renumbering tests: outdent `1. L2` in
      `- L0` / `2. L1` / `1. L2` / `- L3` / `# L4` yields `2. L1` / `3. L2`, with `2. L1`
      byte-identical. This is the reported repro.
- [x] 2.2 Indent the top-level `1. x` under a `- p` whose children are `5. a` / `6. b`, and
      assert `5.` / `6.` / `7.`.
- [x] 2.3 Outdent a node whose own children are `5. c1` / `6. c2` and whose following sibling
...
- [x] 2.4 `insertSubtrees` a parsed `1. pasted` after `5. a` in `5. a` / `6. b` / `7. c`; assert
      `5.` / `6. pasted` / `7.` / `8.`. Plus the fallback: a parsed `3. x` / `4. y` landing
      between bullets keeps `3.` / `4.` (D2).
- [x] 2.5 `moveDown` the `- x` in `5. a` / `- x` / `1. c`; assert the joined run reads
...
- [x] 3.1 Add the "nothing above the operand is rewritten" property (D3), over indent, outdent,
...
- [x] 3.2 Restrict it to documents whose every ordered run is already consecutive from its own
...
- [x] 3.3 Fence "above" at the topmost node the operation RELOCATES — for a reorder, the sibling
...
- [x] 3.4 Guard the property's own reach, the way the depth contract guards its floors: assert a
...
- [x] 4.1 Negative control: restore the minimum-present policy at the five behavioral sites and
      ...
      something else.
- [x] 4.2 Confirm the failure is the marker assertion and not a lost label — the same distinction
      the depth contract draws — so a destroyed subject cannot be mistaken for a numbering defect.
- [x] 4.3 Re-measure at seed 42, 3000 runs and confirm the design's numbers: violations
      11 / 63 / 2 / 3 before, 0 / 0 / 0 / 0 after, with accepted and ordered-marker-above counts
      unmoved (proposal.md — Why).

## 5. Integrate

- [x] 5.1 Run `npm test`, `npm run lint`, `npm run build`. The existing suite is expected to be
      unchanged — 0 of 823 tests moved under the spike — so any failure here is a real
      disagreement with a documented outcome and needs reading, not adjusting. Confirmed against
      the post-`reorder-absorption` baseline of 835: 845 pass, 0 moved.
- [x] 5.2 Real-vault pass on the reported shape: a nested ordered item outdenting into an ordered
      run that does not start at 1, and confirm the line above it is untouched in the editor and
      that undo restores exactly one step. Automated in
      `e2e/specs/20-structural-commands.e2e.ts` rather than done by hand, beside the ordered-run
      renumbering case already there, and negative-controlled: red with the arrival site reverted,
