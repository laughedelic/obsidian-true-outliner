...
placement's cost was still assumed rather than measured (`docs/research/structured-backlinks`,
D8). It has since been measured: placing every source of the hub fixture takes about 2ms
(`docs/research/backlinks-footer-spikes`, S5), and a substring pass over fifty thousand nodes
takes 12ms (`docs/research/search-surfaces`). The rule was a cost decision, and the cost is
gone.

This is also the first of the search changes planned in `docs/research/search-surfaces`: the same
...
- Any surface other than the footer: the search palette and the in-note filter are their own
  changes (`docs/research/search-surfaces`, plan), and this change only prepares what they read.
...
- **Modified**: `src/plugin/footer-model.ts` (hit vocabulary), `src/plugin/footer-filter.ts`
  ...
  (render flow places before it counts when a term is active; match marking),
  `styles/20-backlinks-footer.css` (the mark, and the `is-hit` rename).
...
- **Docs**: `docs/research/structured-backlinks` D8 gains the note that its name-only rule is
  superseded and why.
