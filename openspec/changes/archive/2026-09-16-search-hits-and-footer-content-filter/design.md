...
second consumer, and `docs/research/structured-backlinks` D17 says to share the algebra and
not the renderer.

The measurements this design rests on are in `docs/research/backlinks-footer-spikes` (S5:
placement cost) and `docs/research/search-surfaces` (parse-and-match cost; the plan naming this
change as the foundation for the palette and the in-note filter).
...
`docs/research/structured-backlinks`). Children contribute their whole own text, because a child
row renders it.
...
`matchesText` and nothing else, and every surface follows. `docs/research/search-surfaces`'s
survey records what it costs when surfaces disagree about a query.
...
rendered `buildRows`' output unchanged (`docs/research/search-surfaces`), and what it lacked was
only a name that did not say "reference".
...
- [The header's totals are no longer instant while a term is active — they wait for placement]
  ...
  new ones land rather than a skeleton, per D11 in `docs/research/structured-backlinks`.
