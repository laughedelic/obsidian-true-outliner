...
The measured catalogue (docs/research/zoom-editing-boundary) is unambiguous. Nine gestures mutate content the user
...
BEFORE the change, and it is being asked a question offsets cannot answer: docs/research/zoom-editing-boundary's X2
...
- **The escape test is the scope's invariant, judged over the AFTER state**, and asked in one
  ...
  such formulation fails a measured row, including after-state ones (docs/research/zoom-editing-boundary: one
...
- **Promoting the zoom root out of the list.** Every reference outliner renders a zoomed root as a
  ...
  construction (docs/research/zoom-editing-boundary). That is a rendering change of its own size and it is not
...
- **The heading-merge veto defect.** Backspace at a heading's first content character deletes a `#`
  ...
  without a zoom (docs/research/zoom-editing-boundary). It is base behaviour, not zoom behaviour, and belongs to its
  own change.
