...
- `structural-history-integration`'s rationale for recording a move's cursor is corrected: a
  swap has two equally true descriptions, the aligner picks one by tie-break rather than by
  which node the user acted on, so mapping fails in one direction and happens to succeed in the
  other. Recording is still required — it is what makes the answer the same either way.
