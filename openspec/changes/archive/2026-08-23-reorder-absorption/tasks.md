...
- [x] 1.1 Add `reorder-not-expressible` to `RejectionReason` (src/result.ts) and its cue to
...
- [x] 1.2 In `moveSurgery` (src/ops.ts), before the swap, refuse when either relocated root
...
- [x] 1.3 Leave it inside `moveSurgery` so a group reorder inherits it PER STEP (D2). The group
...
- [x] 2.1 In `tests/depth-contract.test.ts`, add a reorder property that compares EVERY label's
...
- [x] 2.2 Add the deferred move-down row to the contract table and remove the deferral comment at
...
- [x] 2.3 Update `tests/group-ops.test.ts` — "a run keeps its order where a step-at-a-time
...
- [x] 2.4 Behavioural tests in `tests/ops.test.ts`, in the block that owns sibling reordering:
...
- [x] 2.5 Confirm the composition oracle still holds. `composeSequential` (tests/group-oracle.ts)
...
- [x] 3.1 Negative control: disable the refusal and confirm the 2.1 property FAILS for both
...
- [x] 3.2 Negative control the other way: widen the refusal past section level and confirm the
      2.4 nested-reorder test fails, so that test guards the scope rather than riding along.
- [x] 3.3 Re-measure on the labelled generator at seed 42, 3000 runs, and confirm the numbers
...
- [x] 3.4 Pin the placement and its cost: a group reorder whose intermediate step is
