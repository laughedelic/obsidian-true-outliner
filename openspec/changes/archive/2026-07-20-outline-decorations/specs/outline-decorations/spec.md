...
that edit, proving no change-bearing transaction was interposed; added by hardening task
4.2); `tests/decorate.test.ts` ("produces no facts for an empty document or preamble-only
document" — the pure computation has no side effects to begin with).
