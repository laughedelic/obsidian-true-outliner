...
- [x] 10.1 Add the caret-scoped visibility state that looks DOWN from the caret — every guide
  inside the node it is in — and verify in `tests/decorate.test.ts` that it and `ancestors`
  partition a row's guides, and in `51-guides-gradient.e2e.ts` that a childless node draws none.
  Negative control: drop the span check and confirm the sibling subtree lights up. (A third
  state drawing only the node's OWN guide was built and withdrawn: too little between the two to
  earn a rung.)
...
- [x] 10.4 Rename the unit's rungs (`balanced`, `auto`), split the defaults — roomy on desktop,
