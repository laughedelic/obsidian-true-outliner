...
- [ ] 0.1 Re-read `openspec/specs/outline-decorations/spec.md` and confirm the guide requirement
      is the one `lists-on-the-outline-grid` restates ("Indentation guides render every ancestor
      level, including list levels"). If that change has not archived yet, retarget this change's
      delta header at the name actually in the main spec — the rule it states does not change
      either way.

## 1. The extent, in the facts

- [ ] 1.1 Trim a gap line's guides in `computeLineGuides` (src/plugin/decorate.ts): after the
...
- [ ] 1.2 State the reasoning in the doc comment where the current "covers every gap line" text
...
- [ ] 1.3 Confirm the fact shape is unchanged: every gap line still gets a fact (possibly with
...
- [ ] 2.1 A guide stops at its subtree's last content line: a section whose last paragraph is
...
- [ ] 2.2 A guide does not run off the end of the document: the final blank row of a file
      ending inside a nested section carries nothing.
- [ ] 2.3 Interior continuity is untouched: a blank line between two siblings inside the same
...
- [ ] 2.4 A run of blanks closing three nested levels at once, with content at the outermost
...
- [ ] 2.5 A blank line inside an atom is content, not a tail: a fenced code block containing an
      empty line keeps its guides on that row (design D3).
- [ ] 2.6 A list guide takes the same rule as any other: the trailing blank after a nested
      list's last item carries neither the list level's guide nor a bridging ancestor's.
- [ ] 2.7 Negative control for the group: disable the trim and confirm 2.1, 2.2, 2.4 and 2.6
      fail — a test that passes against the old code is testing nothing here.

## 3. The provisional extension (design D4)

- [ ] 3.1 `computeLineGuides` takes the open provisional row and treats it as a content line in
...
- [ ] 3.2 Hand that row over from `factsFor` (src/plugin/decorations.ts), in both branches —
...
- [ ] 3.3 Unit-test the extension: a position opened past a section's last content line gets
...
- [ ] 3.4 Unit-test that the extension adds no DEPTH: the position's row carries exactly the
...
- [ ] 4.1 Add `contentEnd` to `ChainEntry` — the subtree's last own-line, filled in on the way
...
- [ ] 4.2 Unit-test that a `'full'` accent ends on its guide's last row and renders on no blank
...
- [ ] 5.1 E2e in `51-guides-gradient.e2e.ts`: on a fixture whose section ends in blank lines,
...
- [ ] 5.2 E2e a run of blanks closing nested subtrees: every row of the run resolves the same
      layer count, it is lower than the last content row above the run, and it matches the depth
      of the content that follows.
- [ ] 5.3 E2e the provisional extension: with the caret on a position past a section's end its
