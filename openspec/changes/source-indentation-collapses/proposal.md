...
it is a change of its own rather than a fix to a rule. What the pass found is that the run is not
content: markdown reads leading whitespace on these kinds as structure — the same structure the
depth already states — and reading mode renders none of it. `lists-on-the-outline-grid` already
took that step for a list item's own run, stating its width from the item's depth rather than
accepting what the characters measured; every other kind was left out.

## What Changes

- **A non-list line's own source indentation renders at no width.** The depth rules are then the
  only thing positioning it, so a child of a list item lands on its depth's column whatever
  whitespace the file wrote, and a two-space file, a three-space file and a tab file agree.
...
- `outline-decorations`: the grid requirement, which today states a LIST line's indentation width
  from its depth, is extended to every other kind — a non-list line's own source indentation
  contributes no width either, so a node's rendered column is a function of its tree depth alone
  whatever encodes it.
...
- `src/plugin/decorations.ts`: a mark decoration and its own view plugin.
