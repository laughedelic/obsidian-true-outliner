...
- [ ] 1.1 In a running vault with outline mode on, put the caret on a Shift+Enter continuation
...
- [ ] 1.2 Repeat inside a list under a heading (`supplementalDepth` ≥ 1) and confirm the
      difference is exactly the missing `supplementalDepth` margin, as the report describes.
- [ ] 1.3 Repeat for Enter's provisional position at a non-zero depth, recording the caret x
      against a real paragraph at the same depth, and whether the caret overlaps the depth-0
      guide column.
- [ ] 1.4 Append the findings to `docs/research/12-decoration-follow-ups.md` under the gap-line
...
- [ ] 2.1 In `src/ops.ts`, make `splitNode`'s gap-widening branch write the destination scope's
...
- [ ] 2.2 Extend `tests/split.test.ts`: a list item with a paragraph child, split at its end,
...
- [ ] 2.3 Cover the two unchanged shapes in the same file: a top-level paragraph and a heading
      both still produce byte-identical output, with the anchor at column 0.
- [ ] 2.4 Cover a paragraph that is itself a child of a list item: the position it opens is at
      its own level, and typing there makes a sibling, not a top-level node.
- [ ] 2.5 Extend `tests/undo-on-abandon.test.ts` with an indented position: abandoning it leaves
      the document byte-identical, with no whitespace-only line behind.
- [ ] 2.6 Negative control: revert 2.1 locally and confirm 2.2, 2.4, and 2.5 fail for the
      reason stated, then restore it.

## 3. The pure "what would this line become" fact

- [ ] 3.1 Add the pure preview function to `src/plugin/decorate.ts` — given the document text
...
- [ ] 3.2 Cover the new-node case in `tests/decorate.test.ts`: Enter's blank-separated position
      ...
      actually gives.
- [ ] 3.3 Cover the continuation case: Shift+Enter's whitespace-only position under a nested
      list item reports a list-item continuation fact (not a first line) carrying the item's own
      `supplementalDepth`.
- [ ] 3.4 Cover the declines: a line that already has a fact, a blank line in the preamble, a
      preamble-only document, an empty document, and a document whose only blank line precedes
      every node.
- [ ] 3.5 Cover the pure-list invariant: a continuation position inside a list with no non-list
      ancestor reports `supplementalDepth` 0, so it contributes no geometry.
- [ ] 3.6 Pin the guide agreement (design D8): the preview fact's own-shift regime matches the
      regime the existing gap-line guide rule assumed for that same line.

## 4. Render it

- [ ] 4.1 In `src/plugin/decorations.ts`, resolve the provisional fact for the caret's line when
...
- [ ] 4.2 Feed that fact into the line-decoration pass: the caret's gap line takes the full
      `lineDecoration` treatment instead of the guide-only one, and every other gap line is
      untouched.
- [ ] 4.3 Feed it into the marker pass through the existing eligibility and `markerVisibility`
      gates — no special case for the provisional line (design D7).
- [ ] 4.4 Compute the position trail from the preview document when a provisional position is
      open (design D4), so the current-node and ancestor accents describe the node the position
      stands for.
- [ ] 4.5 Confirm no CSS change is needed: the existing `to-decor-block` / `to-decor-list` rules
...
- [ ] 5.1 Add to `e2e/specs/50-decorations.e2e.ts`: Enter at the end of a nested paragraph
...
- [ ] 5.2 Add to the same spec: typing one character on either position changes the line's
      rendered indentation by nothing — the jump this change exists to remove.
- [ ] 5.3 Add to `e2e/specs/52-block-markers-icons.e2e.ts`: Enter's position carries exactly one
...
- [ ] 5.4 Add to `e2e/specs/53-decoration-contracts.e2e.ts`: opening, rendering, and abandoning a
...
- [ ] 5.5 Add to `e2e/specs/30-keyboard-grammar.e2e.ts`: the live keypress-then-type sequence on
      a list item with a paragraph child produces a child, with the existing child still a child.
- [ ] 5.6 Pin the caret into content space in the pure-list byte-identity assertions
...
- [ ] 6.1 Mark the graduated entry in `docs/research/12-decoration-follow-ups.md` (the gap-line
      caret jump) as closed by this change, keeping any residual found in 1.4 as its own item.
- [ ] 6.2 Note in `docs/research/15-enter-and-shift-enter-catalogue.md` that S10's decoration
      half and the E10 encoding defect are answered here, so the catalogue stays the record of
      where each finding landed.
- [ ] 6.3 Run `npm test`, `npm run lint`, `npm run build`, and `npm run test:e2e`; then a manual
      real-vault pass over both reported gestures at several depths, in a pure list and under a
      heading, with each `markerVisibility` value.
