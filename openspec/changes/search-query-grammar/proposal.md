...
- **The off-the-shelf fuzziness is worse than vacuous.** A whole-text subsequence returns 28% of
  the corpus for a three-character query and 16% for a mistyped four-letter word, and still misses
  the intended node in a third to nearly two thirds of cases. Confining the match to a word and
  allowing one edit gives 100% recall on both typo families at a median of 5 hits or fewer.
  Highlighting separates them as sharply: 1.0 to 2.5 marks per hit, one per word found, against
  2.7 to 8.4 scattered over 7.5 to 15 times their own length.
...
- **A bare term matches a word by its opening characters**, and for terms of four characters or
  ...
  characters. The floor is four because the sweep puts it there: three costs nearly three times the
  width at three characters and buys no recall, five loses every typo in a four-letter word.
...
- **A node's words are cached with its tree.** Word matching costs ten times a substring pass
  ...
  `SourceTreeCache` takes 53,000 nodes from 128 ms to 43 ms.
...
- **Ranking and scoring.** One edit of tolerance widens the median whole-word result from 5 hits
  to 10 — twice as many, still 0.6% of the corpus, and not enough to force a score. The reason to leave it is structural: the footer
...
- **Modified**: `src/search.ts` — the query parser, the term rule, the chain match, and the ranges
  ...
  `src/plugin/backlinks-footer.ts` (the mark walk carries a step — in `inline-render.ts` once
...
Off `main`. The mechanical test finds one real overlap, and no dependency in either direction.

- **`in-note-outline-filter` (#93)** shares no source or test file with this change. Its
  `outline-filter-state.ts` consumes `matchNodes` and `matchRanges` through the signatures this
  change keeps (design D2), so it gains the new grammar without an edit of its own.
- **`search-palette` (#95)** shares two files. It moves the footer's mark walk out of
  `src/plugin/backlinks-footer.ts` into a new `src/plugin/inline-render.ts`, which is the code
  task 5.2 edits, and it edits `docs/research/search-surfaces` beside this change's two small
  edits there. Its `vault-search.ts` is a second consumer of `matchNodes` and `matchRanges`, through
  the signatures this change keeps (design D2).

Neither reads what the other adds, so neither stacks. Whichever lands second carries the merge:
if the palette lands first, task 5.2 targets `inline-render.ts` rather than `backlinks-footer.ts`;
if this lands first, the palette's extraction moves a mark walk that already carries its step. A
palette that lands first answers `>` through `matchNodes` with no change of its own, and marks
its lineage rows with the hit step's terms until it adopts the per-step ranges D9 describes.
