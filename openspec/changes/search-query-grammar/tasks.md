...
- [ ] 5.2 Carry the admitting step into the mark walk in `src/plugin/backlinks-footer.ts` so a
      lineage row marks its own step and a hit row marks the last (spec `backlinks-footer`). Verify
