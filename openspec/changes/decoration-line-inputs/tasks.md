...
- [x] 1.1 Capture a baseline of what the decoration layer puts in the DOM for the e2e corpus: for
...
- [x] 1.2 Add to `54-widget-rendered-lines.e2e.ts` the case the proposal names: a widget line and
...
- [x] 2.1 Add `LineRender` and `renderInputs(state, modes)` to `src/plugin/decorations.ts` beside
...
- [x] 2.2 Switch `computeDecorations` to the builder: `lineDecoration` and `gapLineDecoration`
