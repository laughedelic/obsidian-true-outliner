...
- [ ] 2.3 Teach `tests/generators.ts` to produce lone ids after every kind, inside and after list
...
- [ ] 3.1 Replace the direct `lines.length` / `trailingGap.length` classification in `src/caret.ts`,
...
- [ ] 4.1 In `src/ops.ts` and `src/reencode.ts`, re-indent `blockId.line` wherever a node's lines are
