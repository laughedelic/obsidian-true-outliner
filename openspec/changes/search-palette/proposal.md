...
`in-note-outline-filter` is off `main` too, and near enough to touch the same files without
depending on this change or being depended on by it — a stack would charge both for an order
neither needs. Three seams, of which the first has already settled in this change's favour:

- **The match-mark rule.** That change has moved it out of `styles/20-backlinks-footer.css` into
  `styles/10-editor.css`, and in moving it dropped the `.to-backlinks-content` scope for a bare
