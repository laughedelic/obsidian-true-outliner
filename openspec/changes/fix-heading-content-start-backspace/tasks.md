...
- [ ] 1.1 Add a `classify` unit case for Backspace at an ATX heading's content start (the
...
- [ ] 1.2 Add `computeVerdict` unit cases for the two heading vetoes — a heading with a
...
- [ ] 2.1 Admit `heading` alongside `list-item` in `crossesViaChromeDeletion`'s marker-space
      shape in `src/classify.ts`; verify 1.1 now passes.
- [ ] 2.2 Admit `heading` alongside `list-item` in `recognizeMergeIntent`'s marker-space branch
...
- [ ] 2.3 Run `npx vitest run` and verify all pre-existing unit tests still pass, the list-item
      and paragraph merge paths included.

## 3. Cover the real keypress end to end

- [ ] 3.1 Correct `e2e/specs/62-outline-edit-enforcement.e2e.ts`'s existing
...
- [ ] 3.2 Add an e2e case for Backspace inside a heading's `#` run asserting the level changes
