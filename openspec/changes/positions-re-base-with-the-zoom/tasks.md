...
- [ ] 1.1 Add the pure scoped materialization beside the existing provisional helpers in
  ...
  case must fail.
- [ ] 1.2 Unit-test the scoped case against the document in docs/research/12: zoomed to a node
...
- [ ] 1.3 Unit-test the bisecting case: a position opened interior to a multi-line node inside a
...
- [ ] 1.4 Pin the coordinate premise in `tests/projection-decorate.test.ts`: over the generated
...
- [ ] 2.1 Make `computeProvisional` in `src/plugin/decorations.ts` the thin wrapper: resolve
...
- [ ] 2.2 Shift in `factsFor`: in the bisecting branch, decorate and compute guides from the
...
- [ ] 2.3 Re-base the trail in `computeTrail`: hand `computePositionTrail` the scoped document at
...
- [ ] 3.1 Add an e2e case to `e2e/specs/80-outline-zoom.e2e.ts`: zoomed to a node with a hidden
...
- [ ] 3.2 Add the bisecting counterpart to the same spec: a position opened interior to a
...
- [ ] 3.3 Add the caret-leaves case: moving off the position without typing restores exactly the
  pre-position render. Negative control: assert it against a build with 2.2 reverted — the
  restored render must differ.
- [ ] 3.4 Run the zoom and decoration groups narrow (`npm run test:e2e:narrow -- 80-outline-zoom`,
  then the decorations and position-indicator specs) and confirm no existing case regresses.

## 4. Landing

- [ ] 4.1 Mark the docs/research/12 entry closed by this change, keeping its measurement, and
