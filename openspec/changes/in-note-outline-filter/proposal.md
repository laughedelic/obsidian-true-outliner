...
collapses to the matching nodes and the ancestors that lead to them, still editable, live as you
type (`docs/research/search-surfaces`, "What other apps do": Workflowy, Dynalist, RemNote's
...
- **New**: `src/plugin/filter-state.ts` (the query and the frozen match set, as a state field
  ...
  `e2e/specs/8x-outline-filter.e2e.ts`, `styles/70-outline-filter.css`.
...
matcher. The zoom work this change edits around — `zoom-edit-confinement`,
`positions-re-base-with-the-zoom` and the folding stack — has landed, so nothing else is in its
way. Independent of `search-palette`.
