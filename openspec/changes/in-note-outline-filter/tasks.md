...
- [ ] 1.1 Probe many visible spans through the hiding builder on a real instance: an e2e probe
      ...
      `docs/research/search-surfaces` under a new section. Verified by the section
- [ ] 1.2 Walk `docs/research/zoom-editing-boundary`'s gesture catalogue under a frozen anchor
      ...
      expects, and which rows need an anchor added (design D2); record the table in the same
      section. Verified by the table
...
- [ ] 4.3 Styles for the panel and the hidden-child rendering decided in design's open question,
      as a new `styles/70-outline-filter.css` part; move the `to-match` rule from the footer's
      part into `styles/10-editor.css` (design D6); verify by screenshot in both themes

## 5. End-to-end

- [ ] 5.1 Add `e2e/specs/8x-outline-filter.e2e.ts`, with its own helpers beside it as
      `e2e/outline-filter.ts`, covering: the command is absent outside outline mode; a deep match
      keeps its path and hides siblings; a match's children are hidden; hidden lines take no space
      and the caret skips them; marks; title, properties and footer stay; editing a match away
      keeps it; Enter from a visible node creates a visible node; re-running the query re-decides;
      moving a node carries hidden children; filter inside a zoom searches the scope; zooming
      while filtered keeps it; clearing keeps the zoom; no matches shows the note whole with the
      message; closing restores everything. Negative controls: for the frozen-set tests, re-run
      the query on every change; for the caret tests, unregister the resolver; for the composition
      tests, skip the intersection
...
- [ ] 5.3 Manual pass in a real vault under both themes, filtered and zoomed; record findings
      in `docs/research/search-surfaces`

## 6. Docs and validation

- [ ] 6.1 Record in `docs/research/search-surfaces` what the spike changed in the design, if
      anything, and close its open question 3 with the frozen-set decision
