...
- [x] 1.1 Add `tests/depth-contract.test.ts` with a depth reader built on `forEachNodeWithLine`
      ...
      `0` (D4).
- [x] 1.2 Assert the single-node contract over `arbLabeledDoc()`: pick a non-heading subject,
...
- [x] 1.3 Assert the group contract over the same generator: build the operand with
      ...
      delta.
- [x] 1.4 Name the excluded move-down row in a comment that states the defect, its minimal case
      (`- L0` / blank / `L1`), and the change that will add the row (D6).

## 2. Coverage guards

- [x] 2.1 Count accepted cases per operation SEPARATELY for each form, and fail below a floor set
...
- [x] 2.2 Count accepted MULTI-ROOT group operands per operation and guard them on their own
...
- [x] 3.1 Negative control: temporarily add the move-down row and confirm the property FAILS,
...
- [x] 3.2 Confirm the property passes with move down excluded, and that the failure in 3.1 came
