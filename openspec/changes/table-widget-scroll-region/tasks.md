...
- [x] 1.2 Add the two add-button rules (D3) on the same gate — the native near inset released to
...
- [x] 3.4 Check a table in a note with an RTL first line, confirming D3's logical insets put the
      add-column button on the inline-far side rather than over the table

## 4. Land

- [x] 4.1 `npm run lint`, `npm test`, `npm run build`, `npm run build:e2e`
