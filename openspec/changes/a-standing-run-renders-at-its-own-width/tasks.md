...
- [ ] 1.1 Select the quantiser override on the kind classes (`to-decor-block`, `to-decor-atom`)
...
- [ ] 1.2 Remove `SOURCE_INDENT_SIZED_CLASS`, its export and its `Decoration.line` range from
  `computeSourceIndent`, leaving the mark and the replacement untouched.

## 2. What the rule has to hold

- [ ] 2.1 E2E: a top-level four-space line's text begins four space advances into the line, and the
...
- [ ] 2.2 E2E: walking the caret through a standing run steps one space advance per press, with no
  press landing on a box edge. Negative control: the same restoration reintroduces the 20.75px step.
- [ ] 2.3 E2E: a tab-indented top-level line renders its text on the same column as a four-space
  one.
- [ ] 2.4 E2E: the lines whose run is HIDDEN are unmoved — the existing child-column, deeper-line,
...
- [ ] 3.3 Remove the probe spec once its figures are in
