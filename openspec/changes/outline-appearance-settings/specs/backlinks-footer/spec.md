...
The footer SHALL take the outline unit and the guides' appearance — thickness and intensity —
from the same declarations the editor reads, so that a note and the references below it render
...
The two visibility qualifiers that depend on the open note SHALL NOT apply to the footer. The
footer has no caret of its own, and every row's lineage begins at its own source note's root, so
neither "the levels the cursor is inside" nor "the document has a single root" names anything
there. A footer row SHALL therefore draw one guide per ancestor row above it whatever the editor's
...
- **WHEN** the guides' thickness or intensity changes while the footer is drawing guides
- **THEN** the footer's guides render at the new thickness and intensity, matching the editor's
