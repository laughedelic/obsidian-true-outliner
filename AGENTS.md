...
- **No agent attribution trailers** in commit messages or PR descriptions; `.claude/settings.json`
  ...
  appends a `Generated by [Claude Code]` footer with the session URL regardless of those settings;
  a `PostToolUse` hook strips it, and a body rewritten afterwards keeps its text. A PR opened
  through REST (`gh api repos/{owner}/{repo}/pulls`) never gets one.
...
the refusal of a push or PR on a `claude/*` head, and the PR-footer strip. Copilot reads neither,
