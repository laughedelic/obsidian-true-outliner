...
an item's TEXT first and the beginning of its checkbox second, and Mod-Backspace should delete
through the same two stops.

The measurement that settles the shape is already in Q26's own follow-up table, taken on a checkbox
item:
...
So the ladder being asked for is the one cmd+Left already walks, and the key that disagreed with it
was ours. Home now walks the same two columns, which makes the request a consistency fix rather than
a new design: nothing is added to cmd+Left, and Q26's decision to leave `Mod-ArrowLeft` unbound
stands — binding it would still override a native ladder for no invariant gain.

**Why this is not the escalation Q26 retired.** Those ladders climbed across LINES and across
RENDERED GEOMETRY — visual row, raw line, node — so a press meant different things depending on
where the previous one had landed and where the renderer happened to wrap. The task stop shares none
of that: it is one line, the checkbox is drawn on it, and the caret's own column decides which stop
is next. The rule stays computable from the parsed line, which is what Q26 was protecting.

**Both keys read one helper.** `contentStartRungs` (caret-policy.ts) returns the pair — where the
text begins, and the boundary in front of the box — and collapses it on every line that carries no
task marker, which is every kind but a task item's first. Home moves through the pair;
`planDeleteToContentStart` deletes through it. A caret INSIDE the marker is at neither stop: Home
takes the nearer one, as it always did, and the deletion stays a Backspace there, since a range from
the boundary would leave `- ] text`.
