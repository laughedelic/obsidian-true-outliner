...
`--checkbox-size / 2` — a theme's value, not ours, and font-independent. The single-digit
ordered number is second at +7.40px and is font-*dependent*: on the wider Linux font CI runs,
`1. ` measures 20.36px against 18.39px here ([16](16-native-list-decoration.md)), which puts
its ink at roughly +8.9px and makes it the binding mark instead. Both are covered; which one
binds depends on the reader's font.
...
**0.375rem (6px at a 16px root)**, giving a gutter of **8.00 + 6.00 = 14px = 0.875rem**.
...
2. **It is the tightest value the mechanisms actually survive.** At a 0.75rem gutter the task
   label's `min-width` falls to 7.81px, under the checkbox's own 8.00px advance, and a task's
   text leaves the shared column while every other kind's stays on it. 0.875rem is the last
   step before that.
...
Three, all of the same shape: a value that was inert only because the old gutter was
large enough to dominate it.
