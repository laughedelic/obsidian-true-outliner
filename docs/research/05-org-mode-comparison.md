...
| Selection closure | region selection is plain text — no structural closure at all | a selection is always a forest of whole subtrees, closed DOWNWARD only: no node is selected without its children, but a node may be selected together with content outside its parent | outliner-UX convention — Logseq, Workflowy, Roam, Notion, Dynalist and Tana all enforce downward closure and none enforces the upward half (`selection-as-subtree-set`, 2026-07-31) |
