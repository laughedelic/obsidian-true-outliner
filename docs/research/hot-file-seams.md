...
| 3 | One builder of a render's line inputs — **landed** as `decoration-line-inputs`: `renderInputs` builds one record per line, both consumers read it, and the four per-line helpers have one caller | `computeDecorations` and `MarginCompensation` read inputs computed once | 9 of the 13 same-code conflicts in `decorations.ts` | refactor, not a move |
