...
/** The user-supplied branching shape from docs/research/structured-backlinks, D4. */
const BRANCHING = `- a
\t- b
\t\t- x first mention
\t\t- c
...
describe('lineage: collapsing', () => {
  it('collapses each sub-branch independently, not only the common prefix', () => {
    // The exact shape and expected output from docs/research/structured-backlinks, D4.
    const doc = parse(BRANCHING);
