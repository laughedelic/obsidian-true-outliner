...
function caretScope(state: EditorState): CaretGuideScope | null {
  ...
  // render at source-document depths, the gap `docs/research/decoration-follow-ups` recorded.
...
// `MarkerVisibility` itself lives in settings/appearance.ts (not here), so that
...
// `editorInfoField`).
export type { MarkerVisibility } from './settings/appearance';
import type { MarkerVisibility } from './settings/appearance';
 ...
 * follow-up — see `MarkerVisibility`'s own doc comment in settings/appearance.ts
 ...
 * reader has (docs/research/fold-mechanics).
 *
